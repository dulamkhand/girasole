...
class QuizQuestionTable extends Doctrine_Table
{
    ...
    public function getPager($params = array(), $page=1)
    {
        ...
        $pager = new sfDoctrinePager('QuizQuestion', sfConfig::get('app_pager', 30));
        $pager->setPage($page);
        $pager->setQuery($q);
        $pager->init();
    ...
    private function params($q, $params = array())
    {
        ...
        if(isset($params['number']) && $params['number'] != null)
            $q->andWhere('number= ?', $params['number']);
        ...
        $order = isset($params['orderBy']) ? $params['orderBy'] : 'sort DESC, number ASC, created_at DESC';
        $q->orderBy($order);
