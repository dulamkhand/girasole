...
class GlobalTable extends Doctrine_Table
{
    ...
    private static function params($tableName, $q, $params = array())
    {
        ...
        if(isset($params['isUsed']) && in_array($params['isUsed'], array('0', '1'))) 
            $q->andWhere('is_used = ?', $params['isUsed']);

        # keyword
        if(isset($params['keyword']) && $params['keyword'] != null) {
        		if($tableName == 'Feedback') {
		        		$q->andWhere('message LIKE ? OR name LIKE ?', array('%'.$params['keyword'].'%', '%'.$params['keyword'].'%'));
        		} else {
